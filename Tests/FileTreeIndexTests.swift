...
final class FileTreeIndexTests: XCTestCase {
    ...
    func testDirectoryListingIncludesDotfilesByDefault() async throws {
        let root = try makeTempDirectory()
        let dotfile = root.appendingPathComponent(".env")
        let visible = root.appendingPathComponent("visible.txt")
        try Data("x".utf8).write(to: dotfile)
        try Data("y".utf8).write(to: visible)

        let index = FileTreeIndex(rootPath: root.path)
        let rootChildren = try await loadChildrenAndWait(index, path: root.path)
        XCTAssertTrue(rootChildren.contains(where: { $0.name == ".env" && !$0.isDirectory }))
        XCTAssertTrue(rootChildren.contains(where: { $0.name == "visible.txt" && !$0.isDirectory }))
    }
    ...
    func testSearchDefaultsIncludeHiddenPaths() async throws {
        let root = try makeTempDirectory()
    ...
    func testSearchCanExcludeHiddenPathsWhenDisabled() async throws {
        let root = try makeTempDirectory()
        let hidden = root.appendingPathComponent(".env")
        try Data("TOKEN=x".utf8).write(to: hidden)

        let index = FileTreeIndex(rootPath: root.path)
        let options = FileTreeIndex.SearchOptions(includeHiddenEntries: false)
    ...
    func testSearchIncludedHiddenEntriesAreRankedAfterVisibleMatches() async throws {
        ...
        let index = FileTreeIndex(rootPath: root.path)
        let results = await index.search(matching: "root", limit: 20)
