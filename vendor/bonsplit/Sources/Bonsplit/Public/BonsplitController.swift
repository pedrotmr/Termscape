...
public final class BonsplitController {
    ...
    public func createTab(
        ...
        let targetPane =
            pane ?? focusedPaneId ?? PaneID(id: internalController.rootNode.allPaneIds.first!.id)
    ...
    public func requestTabContextAction(
        _ action: TabContextAction, for tabId: TabID, inPane pane: PaneID
    ) {
    ...
    public func splitPane(
        ...
        delegate?.splitTabBar(
            self, didSplitPane: targetPaneId, newPane: newPaneId, orientation: orientation
        )
    ...
    /// Replaces the split tree with a snapshot from `treeSnapshot()`, preserving pane and tab UUIDs.
    public func replaceRootTree(with snapshot: ExternalTreeNode, focusedPaneId: PaneID? = nil) {
        internalController.rootNode = SplitTreeRestoration.splitNode(from: snapshot)
        internalController.clearPaneZoom()
        if let focus = focusedPaneId, internalController.rootNode.findPane(focus) != nil {
            internalController.focusedPaneId = focus
        } else if let first = internalController.rootNode.allPaneIds.first {
            internalController.focusedPaneId = first
        } else {
            internalController.focusedPaneId = nil
        }
        delegate?.splitTabBar(self, didChangeGeometry: layoutSnapshot())
    }

    private func buildExternalTree(
        from node: SplitNode, containerFrame: CGRect,
        ...
        case let .split(splitState):
            ...
            case .horizontal:
                ...
                secondBounds = CGRect(
                    x: bounds.minX + bounds.width * dividerPos, y: bounds.minY,
            ...
            case .vertical:
                ...
                secondBounds = CGRect(
                    x: bounds.minX, y: bounds.minY + bounds.height * dividerPos,
            ...
            let splitNode = ExternalSplitNode(
                ...
                first: buildExternalTree(
                    from: splitState.first, containerFrame: containerFrame, bounds: firstBounds
                ),
                second: buildExternalTree(
                    from: splitState.second, containerFrame: containerFrame, bounds: secondBounds
                )
            )
    ...
    @discardableResult
    public func setDividerPosition(
        _ position: CGFloat, forSplit splitId: UUID, fromExternal: Bool = false
    ) -> Bool {
    ...
    func notifyGeometryChange(isDragging: Bool = false) {
        ...
        if isDragging {
            ...
            guard now - internalController.lastGeometryNotificationTime >= debounceInterval else {
                return
            }
