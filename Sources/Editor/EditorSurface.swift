...
private enum EditorPaneAlert: Identifiable, Equatable {
  ...
  var id: String {
    switch self {
    case .closeDirty(let id, _): "close-\(id.uuidString)"
    case .saveConflict(let id, _, let close): "conflict-\(id.uuidString)-\(close)"
    case .userMessage(let title, let detail): "msg-\(title)-\(detail.hashValue)"
    }
  }
}
...
struct EditorSurfaceRootView: View {
  ...
  private var editorAlertTitle: String {
    ...
    case .userMessage(let title, _): title
    case .none: ""
    }
  }
