...
private final class BrowserAddressField: NSTextField {
    ...
    private func configureSingleLinePresentation() {
        guard let cell = self.cell as? NSTextFieldCell else { return }
...
final class BrowserSurfaceContainerView: NSView {
    ...
    private func setupLayout() {
        ...
        NSLayoutConstraint.activate([
            ...
            developerToolsButton.trailingAnchor.constraint(equalTo: toolbarView.trailingAnchor, constant: -8),
            ...
            addressField.trailingAnchor.constraint(equalTo: developerToolsButton.leadingAnchor, constant: -8),
...
@MainActor
final class BrowserSurface: NSObject, Identifiable, WKNavigationDelegate {
    let id: UUID
    ...
    private func setDeveloperToolsVisible(_ visible: Bool) -> Bool {
        if visible {
            if let inspector = webView.termscapeInspectorObject() {
                ...
                let shown = inspector.termscapeCallVoid(selector: NSSelectorFromString("show"))
                    || inspector.termscapeCallVoid(selector: NSSelectorFromString("open"))
                if shown {
                    return true
                }
            }
            return webView.termscapeCallVoid(selector: NSSelectorFromString("_showWebInspector:"), object: nil)
        }

        if let inspector = webView.termscapeInspectorObject() {
            let hidden = inspector.termscapeCallVoid(selector: NSSelectorFromString("close"))
        ...
        return webView.termscapeCallVoid(selector: NSSelectorFromString("_closeWebInspector:"), object: nil)
    }
