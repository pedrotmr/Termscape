...
final class GhosttyApp {
  ...
  private func initializeGhostty() {
    ...
    runtimeConfig.close_surface_cb = { userdata, needsConfirmClose in
      ...
      DispatchQueue.main.async {
        ...
        guard
          let workspace = appState.groups.lazy.flatMap(\.workspaces)
            .first(where: { $0.id == workspaceId })
        else { return }
        for tab in workspace.tabs {
          if let (tabKey, terminalSurface) = tab.surfaces.first(where: { $0.value.id == surfaceId })
          {
            terminalSurface.teardown()
            tab.surfaces.removeValue(forKey: tabKey)
            return
          }
        }
      }
  ...
  private nonisolated static func handleRuntimeAction(
